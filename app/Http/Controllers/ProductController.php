...
class ProductController extends Controller
{
    ...
    public function payNotify()
    {
        ...
        \DB::transaction(function () use ($order, $data) {
            ...
            if ($enterProduct) {
                if (Carbon::now()->diffInDays($enterProduct->expiry_date,false) < 0) {
            ...
            $order->update([
                ...
                'remark' => '原到期日:' . $source_date->format('Y-m-d') . ',当前到期日:' .  $expiry_date->format('Y-m-d'),
            ]);
        });
        return app('alipay')->success();
    }
